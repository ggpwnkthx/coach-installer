        ...
        then
          ...
          then
            ...
            then
              ...
              if [[ $noselect = 1 ]]
              then
                printf  "$dev_id	HDD	${RED}(osd.$osd_id)${NC}\n"
              else
...
menu_ceph_osd()
{
  ...
  printout_ceph_osd
  printf '%*s\n' "${COLUMNS:-$(tput cols)}" '' | tr ' ' -
  echo "[A] All Available (No Separate Journaling)"
  printf '%*s\n' "${COLUMNS:-$(tput cols)}" '' | tr ' ' -
  echo "[0]	BACK"
  echo ''
}
...
remove_ceph_osd()
{
  ...
  then
    ...
    then
      ...
      then
        osd_id=$(menu_ceph_osd | grep '\['$to_be_osd'\]' | awk '{print $4}' | tr -d '()' | awk '{split($0,a,"."); print (0+a[2])}')
